...
router.get("/businessplans/:businessid",(req,res)=>{
    let id = req.params.businessid;
    Businessplan.find({businessid: id}).populate({path:"businesses",select:["name"]}).then(result=>{
        res.end(JSON.stringify({status : "success", data : result}));
    }).catch(err=>{
        res.end(JSON.stringify({status : "failed", data : "Record not found"}));        
    })
...
router.post("/businessplans",(req,res)=>{
    let businessplans = new Businessplan(req.body);
    businessplans.save().then(result=>{
        res.end(JSON.stringify({status : "success", data : result}));
    }).catch(err=>{
        res.end(JSON.stringify({status : "failed", data : "Record not found"}));        
    })
